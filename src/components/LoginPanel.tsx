...
import { useImportFromBrowser, useLoginByCookie } from '../hooks/useAuth.ts';
...
export function LoginPanel({ notice }: { notice?: string }) {
  ...
  const error = login.error ?? importFromBrowser.error;
  const busy = login.isPending || importFromBrowser.isPending;
    ...
    <div className="mx-auto flex min-h-full max-w-3xl flex-col justify-center gap-8 px-6 py-12">
      <header className="space-y-2">
        ...
        <p className="text-sm text-zinc-400">
          把网易云歌单里的音频、封面、歌词和简介落到本地磁盘。先填一次 MUSIC_U。
        </p>
      </header>
      ...
      <form onSubmit={submit} className="space-y-3">        <label htmlFor="music-u" className="flex items-center gap-2 text-sm font-medium">
        ...
        <div className="flex flex-wrap items-center gap-3">
          ...
          <span className="text-xs text-zinc-500">
            导入需要额外装 @steipete/sweet-cookie，而且 macOS 会弹钥匙串授权
          </span>
        </div>
      </form>
