...
// 逐字复制自 neteasecli/src/api/crypto.ts —— 这份实现是验证过的，别动。
// 唯一的改动是 'crypto' → 'node:crypto'。
...
// 网易的 RSA 有三个非标准之处，缺一个都出不来正确的 encSecKey：
// 反转明文、左侧补零到 128 字节、不用 padding。
...
// linuxapi —— Linux 客户端通道。当前无调用方，保留备用
...
// eapi —— 移动端通道。当前无调用方，M4 探索单曲百科接口时会用到
